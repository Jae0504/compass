...
K=10
EF_LIST="32,64,96,128,160,200"
# EF_LIST_1PCT="608,672,736,880,1040,1280,1600,1920"
EF_LIST_1PCT="800"
EF_LIST_10PCT="6800,8000, 9600, 11200, 12800, 14400, 16000, 19200, 22400"
EF_LIST_SET_BY_USER=0
IN_SEARCH_EF_LIST_1PCT="4,12,14,20"
IN_SEARCH_EF_LIST_10PCT="20,24,32,40,48,56"
...
set_dataset_defaults() {
  case "$DATASET" in
    ...
    sift1b)
      ...
      : "${GRAPH_PATH:=/storage5/jykang5/sift1b_hnsw/bigann_base_m44_efc200.bin}"
      : "${QUERY_PATH:=/storage5/jykang5/sift1b_dataset/bigann_query.bvecs}"
      : "${MANIFEST_1PCT:=/fast-lab-share/jykang5/spr4_storage/fid_tb/n_filter_100/sift1b/manifest.json}"
      : "${MANIFEST_10PCT:=/fast-lab-share/jykang5/spr4_storage/fid_tb/n_filter_10/sift1b_merge_factor2/manifest.json}"
...
run_single() {
  ...
  case "$method" in
    ...
    in_search_filter_hnsw)
      ...
      if [[ "$HNSW_DATASET_TYPE" == "sift" ]]; then
        local nfilters
        nfilters="$(python3 -c "import json; print(json.load(open('$manifest_path'))['nfilters'])")"
        cmd+=(--nfilters "$nfilters")
      else
        cmd+=(--payload-jsonl "$PAYLOAD_JSONL")
      fi
      ;;
