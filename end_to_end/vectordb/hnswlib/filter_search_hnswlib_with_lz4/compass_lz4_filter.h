...
inline Lz4BlockStorage compress_to_lz4_blocks(
    ...
    for (size_t bid = 0; bid < nblocks; ++bid) {
        ...
        const int bound = LZ4_compressBound(raw_len_i);
        if (bound <= 0) {
        ...
        out.compressed_blocks[bid].resize(static_cast<size_t>(csize));
        out.raw_block_sizes[bid] = static_cast<uint32_t>(raw_len);
    }
...
inline uint8_t read_raw_byte(
    ...
    const std::vector<uint8_t>& block = get_or_decompress_block(storage, block_id, is_fid, cache, metrics);
...
class CompassLz4FilterEngine {
public:
    static CompassLz4FilterEngine Build(
...
private:
    struct AttributeStorage {
        ...
        detail::Lz4BlockStorage fid_blocks;
        detail::Lz4BlockStorage tb_blocks;
    };
