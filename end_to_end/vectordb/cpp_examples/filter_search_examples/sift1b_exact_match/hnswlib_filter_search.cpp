...
struct Args {
    ...
    int nfilters = -1;  // when > 0, compute synthetic_id_bucket on-the-fly (no CSV needed)
...
Args parse_args(int argc, char** argv) {
    ...
    if (args.nfilters != -1 && args.nfilters <= 0) {
        throw std::runtime_error("--nfilters must be > 0");
    }
    if (args.dataset_type == "sift" && args.nfilters <= 0) {
        ensure_readable_file(args.metadata_csv, "--metadata-csv");
    } else if (args.dataset_type != "sift") {
...
class ComputedBucketFilterFunctor : public hnswlib::BaseFilterFunctor {
public:
    ComputedBucketFilterFunctor(size_t n_elements, int nfilters, const filter_expr::Expression& expr)
        : nfilters_(static_cast<size_t>(nfilters)), expr_(expr) {
        max_per_ = (n_elements + nfilters_ - 1) / nfilters_;
    }
    ...
    bool operator()(hnswlib::labeltype label) override {
        ...
        const size_t bucket = std::min(static_cast<size_t>(label) / max_per_, nfilters_ - 1);
        const std::string bucket_str = std::to_string(bucket);
        auto accessor = [&](const std::string& field) -> std::optional<std::string_view> {
            if (field == "synthetic_id_bucket") return std::string_view(bucket_str);
            return std::nullopt;
        };
        const bool matched = expr_.evaluate(accessor);
    ...
    uint64_t eval_calls() const { return eval_calls_; }
    uint64_t eval_time_ns() const { return filter_eval_time_ns_; }
...
RunStats run_search_typed(
    ...
    // Build filter functor: computed bucket mode or metadata lookup mode.
    const bool use_computed = (args.nfilters > 0);
    const size_t nfilters_sz = use_computed ? static_cast<size_t>(args.nfilters) : 0;
    const size_t max_per = use_computed ? (total_elements + nfilters_sz - 1) / nfilters_sz : 0;

    auto compute_matches = [&](hnswlib::labeltype label) -> bool {
        const size_t bucket = std::min(static_cast<size_t>(label) / max_per, nfilters_sz - 1);
        const std::string bucket_str = std::to_string(bucket);
        auto accessor = [&](const std::string& field) -> std::optional<std::string_view> {
            if (field == "synthetic_id_bucket") return std::string_view(bucket_str);
            return std::nullopt;
    ...
    ExpressionFilterFunctor expr_functor(metadata, expr);
    ComputedBucketFilterFunctor comp_functor(total_elements, args.nfilters > 0 ? args.nfilters : 1, expr);
    hnswlib::BaseFilterFunctor& filter_functor = use_computed
        ? static_cast<hnswlib::BaseFilterFunctor&>(comp_functor)
        : static_cast<hnswlib::BaseFilterFunctor&>(expr_functor);
    auto get_eval_calls = [&]() -> uint64_t {
        return use_computed ? comp_functor.eval_calls() : expr_functor.eval_calls();
    };
    auto get_eval_time_ns = [&]() -> uint64_t {
        return use_computed ? comp_functor.eval_time_ns() : expr_functor.eval_time_ns();
    };
    ...
    for (size_t internal = 0; internal < total_elements; ++internal) {
        ...
        const bool matched = use_computed
            ? compute_matches(label)
            : metadata_matches_filter(label, metadata, expr);
        if (matched) {
    ...
    uint64_t prev_filter_time_ns = get_eval_time_ns();
    for (size_t qid = 0; qid < query_count; ++qid) {
        ...
        const uint64_t cur_filter_time_ns = get_eval_time_ns();
    ...
    stats.filter_eval_calls = get_eval_calls();
    stats.filter_eval_time_ns = get_eval_time_ns();
...
int main(int argc, char** argv) {
    try {
        ...
        if (args.nfilters > 0) {
            // Computed bucket mode: skip CSV/JSONL loading entirely.
        } else if (args.dataset_type == "sift") {
