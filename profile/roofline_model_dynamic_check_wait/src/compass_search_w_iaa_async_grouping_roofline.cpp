...
std::vector<std::pair<DistT, hnswlib::labeltype>> search_with_async_eq_filter(
    ...
    if (kMeasureInSearchStats) {
        call_stats->iaa_tb_submit_no_flush_ns += static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    auto submit_pending_fid_blocks = [&]() -> bool {
        ...
        if (kMeasureInSearchStats) {
            call_stats->iaa_fid_submit_no_flush_ns += static_cast<uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    auto retry_temp_buffer_once = [&]() {
        ...
        for (const FrontierCandidate& entry : temp_result_buffer) {
            ...
            const bool fid_match = fid_match_node(
                ...
                (kMeasureInSearchStats ? &call_stats->decomp : nullptr),
                (kMeasureInSearchStats ? &call_stats->iaa_fid_wait_ns : nullptr));
            if (kMeasureInSearchStats) ++call_stats->filter_eval_calls;
            if (fid_match) {
                push_result_candidate_if_allowed(entry);
            }
    ...
    while (cache->tb_query_mask_ready == 0) {
        ...
        const uint64_t tb_wait_ns = ring->wait_one_wait_ns_only();
        if (kMeasureInSearchStats) {
            call_stats->iaa_tb_wait_ns += tb_wait_ns;
        }
    }
...
RunStats run_search_typed(
    ...
    for (size_t qid = 0; qid < query_count; ++qid) {
        ...
        stats.iaa_tb_wait_ns += call_stats.iaa_tb_wait_ns;
        stats.iaa_fid_submit_no_flush_ns += call_stats.iaa_fid_submit_no_flush_ns;
        stats.iaa_fid_wait_ns += call_stats.iaa_fid_wait_ns;
        ...
        if (capture_per_query) {
            ...
            m.iaa_tb_wait_ns = call_stats.iaa_tb_wait_ns;
            m.iaa_fid_submit_no_flush_ns = call_stats.iaa_fid_submit_no_flush_ns;
            m.iaa_fid_wait_ns = call_stats.iaa_fid_wait_ns;
            ...
            per_query_out
                ...
                << call_stats.iaa_tb_wait_ns << ','
                << call_stats.iaa_fid_wait_ns
                << '\n';
        }
...
std::string build_summary(
    ...
    oss << "iaa_tb_wait_no_flush_ms: " << (static_cast<double>(stats.iaa_tb_wait_ns) / 1e6) << "\n";
    oss << "iaa_tb_wait_no_flush_ns: " << stats.iaa_tb_wait_ns << "\n";
    ...
    oss << "iaa_fid_wait_no_flush_ms: " << (static_cast<double>(stats.iaa_fid_wait_ns) / 1e6) << "\n";
    oss << "iaa_fid_wait_no_flush_ns: " << stats.iaa_fid_wait_ns << "\n";
    oss << "iaa_tb_wait_ms: " << (static_cast<double>(stats.iaa_tb_wait_ns) / 1e6) << "\n";
    oss << "iaa_fid_wait_ms: " << (static_cast<double>(stats.iaa_fid_wait_ns) / 1e6) << "\n";
    oss << "iaa_tb_wait_ns: " << stats.iaa_tb_wait_ns << "\n";
    oss << "iaa_fid_wait_ns: " << stats.iaa_fid_wait_ns << "\n";
