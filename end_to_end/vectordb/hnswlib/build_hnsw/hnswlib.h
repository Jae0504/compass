...
class AlgorithmInterface {
 public:
    ...
    virtual std::priority_queue<std::pair<dist_t, labeltype>>
        searchKnn(const void*, size_t, std::vector<qpl_job *>* job = nullptr, int pre_decompressed_job_idx = 0,  BaseFilterFunctor* isIdAllowed = nullptr) const = 0;
...
AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void* query_data, size_t k,
    ...
    auto ret = searchKnn(query_data, k, nullptr, 0, isIdAllowed);
    {
        size_t sz = ret.size();
        result.resize(sz);
        while (!ret.empty()) {
