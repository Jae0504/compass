...
AsyncMultiEqRuntime build_async_multi_eq_runtime(
    ...
    std::vector<filter_expr::detail::Token> tokens = filter_expr::detail::tokenize(args.filter_expression);
    ...
    std::function<std::unique_ptr<MultiEqNodeRuntime>(const filter_expr::Node*)> compile =
        [&](const filter_expr::Node* node) -> std::unique_ptr<MultiEqNodeRuntime> {
            if (node == nullptr) {
                throw std::runtime_error("Expression tree contains null node");
            }
            auto out = std::make_unique<MultiEqNodeRuntime>();
            if (node->kind == filter_expr::Node::Kind::Logical) {
                if (node->logical_op != filter_expr::LogicalOp::And &&
                    node->logical_op != filter_expr::LogicalOp::Or) {
                    throw std::runtime_error("Only AND/OR logical operators are supported in async multiple-em");
                }
                out->kind = MultiEqNodeRuntime::Kind::Logical;
                out->logical_op = node->logical_op;
                out->left = compile(node->left.get());
                out->right = compile(node->right.get());
                return out;
            }
            ...
            if (node->kind != filter_expr::Node::Kind::Compare ||
                ...
                throw std::runtime_error(
                    "Async multiple-em supports only equality leaves combined by AND/OR");
            }

            EqPredicateSpec spec;
            spec.field = node->field;
            spec.literal = node->literal;
            runtime.leaves.push_back(build_leaf_runtime(spec));

            out->kind = MultiEqNodeRuntime::Kind::Leaf;
            out->leaf_index = runtime.leaves.size() - 1;
            return out;
        };

    runtime.root = compile(root.get());
    if (runtime.leaves.size() < 2) {
        throw std::runtime_error(
            "Async multiple-em requires at least two equality leaves combined with AND/OR");
    }
    return runtime;
}
